...
// x,y : 起点坐标	 
// num : 数值 (0 ~ 0xFFFFFFFF)
// len : Hex位数 (例如 32 位整数最大显示 8 位 Hex)
// size: 字体大小
void LCD_ShowHexNum(u16 x, u16 y, u32 num, u8 len, u8 size)
{ 	 	
    u8 t, temp;
    u8 enshow = 0;						   
    ...
    for(t = 0; t < len; t++)
    {
        // 每次取出一个 16 进制位 (4 个 bit)，从高位到低位提取
        temp = (num >> (4 * (len - t - 1))) & 0x0F;
        
        // 高位消零逻辑（如果是前导 0 则输出空格）
        if(enshow == 0 && t < (len - 1))
        {
            if(temp == 0)
            {
        ...
        {
            LCD_ShowChar(x + (size / 2) * t, y, temp - 10 + 'A', size, 0); // 若需小写改为 'a'
        }
    }
}
